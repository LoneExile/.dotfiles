...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    _module.args.debugLevel = 
      if cfg.debugLevel == "none" then errorLib.debug.levels.NONE
      else if cfg.debugLevel == "error" then errorLib.debug.levels.ERROR
      else if cfg.debugLevel == "warn" then errorLib.debug.levels.WARN
      else if cfg.debugLevel == "info" then errorLib.debug.levels.INFO
      else if cfg.debugLevel == "debug" then errorLib.debug.levels.DEBUG
      ...
      (pkgs.writeShellScriptBin "nix-config-debug" ''
        ...
        nix eval --impure --expr "
          let
            ...
            moduleConfig = builtins.foldl' (acc: part: acc.\${part}) config pathParts;

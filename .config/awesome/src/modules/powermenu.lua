...
return function(s)
  ...
  local lock_command = function()
    ...
    awful.spawn.with_shell("/bin/bash -c $HOME/.config/scripts/i3lock-fancy/lock.sh")
    awesome.emit_signal("module::powermenu:hide")
  end
  ...
  local shutdown_button = button("Shutdown", icondir .. "shutdown.svg", color["Grey500"], shutdown_command)
  local reboot_button = button("Reboot", icondir .. "reboot.svg", color["Grey500"], reboot_command)
  local suspend_button = button("Suspend", icondir .. "suspend.svg", color["Grey500"], suspend_command)
  local logout_button = button("Logout", icondir .. "logout.svg", color["Grey500"], logout_command)
  local lock_button = button("Lock", icondir .. "lock.svg", color["Grey500"], lock_command)

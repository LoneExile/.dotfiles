...
{ lib, pkgs, testFramework, ... }:
let
  # Create a test runner for a specific test suite
  mkTestRunner = { name, tests, description ? "Test suite: ${name}" }:
  ...
  mkParallelTestRunner = { name, tests, maxJobs ? 4, description ? "Parallel test suite: ${name}" }:
  ...
  mkFilteredTestRunner = { name, tests, filter ? null, description ? "Filtered test suite: ${name}" }:
    let
      filteredTests = if filter != null 
  ...
  mkTimedTestRunner = { name, tests, timeout ? 300, description ? "Timed test suite: ${name}" }:

...
let
  ...
  assertions = {
    ...
    assertTrue = name: value: 
      if value 
      then { success = true; message = "${name}: assertion passed"; }
      else { success = false; message = "${name}: expected true, got ${toString value}"; };
    
    # Assert that a value is false
    assertFalse = name: value:
      if !value
      then { success = true; message = "${name}: assertion passed"; }
      else { success = false; message = "${name}: expected false, got ${toString value}"; };
    
    # Assert equality
    assertEqual = name: expected: actual:
      if expected == actual
      then { success = true; message = "${name}: assertion passed"; }
      else { success = false; message = "${name}: expected ${toString expected}, got ${toString actual}"; };
    
    # Assert that a value is not null
    assertNotNull = name: value:
      if value != null
      then { success = true; message = "${name}: assertion passed"; }
      else { success = false; message = "${name}: expected non-null value"; };
    
    # Assert that an attribute exists
    assertHasAttr = name: attr: set:
      if lib.hasAttr attr set
      then { success = true; message = "${name}: attribute '${attr}' exists"; }
      else { success = false; message = "${name}: attribute '${attr}' missing from set"; };
    
    # Assert that a list contains an element
    assertContains = name: element: list:
      if lib.elem element list
      then { success = true; message = "${name}: list contains ${toString element}"; }
      else { success = false; message = "${name}: list does not contain ${toString element}"; };
    
    # Assert that a string matches a pattern
    assertMatches = name: pattern: string:
      if builtins.match pattern string != null
      then { success = true; message = "${name}: string matches pattern"; }
      else { success = false; message = "${name}: string '${string}' does not match pattern '${pattern}'"; };
  };
  ...
  moduleUtils = {
    ...
    testModuleStructure = modulePath: moduleConfig:
      let
        hasEnable = lib.hasAttr "enable" moduleConfig;
        enableType = if hasEnable then lib.typeOf moduleConfig.enable else null;
        
        hasPackage = lib.hasAttr "package" moduleConfig;
        packageType = if hasPackage then lib.typeOf moduleConfig.package else null;
        
        hasSettings = lib.hasAttr "settings" moduleConfig;
        settingsType = if hasSettings then lib.typeOf moduleConfig.settings else null;
    ...
    testModuleEvaluation = modulePath: moduleFile:
      let
        testConfig = {
          imports = [ moduleFile ];
    ...
    testModuleConfigurations = modulePath: moduleFile: configs:
      lib.flatten (lib.mapAttrsToList (configName: config:
        let
          testConfig = {
            imports = [ moduleFile ];
            modules = lib.setAttrByPath (lib.splitString "." modulePath) 
  ...
  configUtils = {
    ...
    testProfileCombinations = profiles:
      lib.flatten (lib.mapAttrsToList (profileName: profileConfig:
        let
          testConfig = {
  ...
  resultUtils = {
    ...
    runAssertions = testName: assertions:
      let
    ...
    mkTestCommand = testName: assertions: pkgs.writeShellScript "test-${testName}" ''

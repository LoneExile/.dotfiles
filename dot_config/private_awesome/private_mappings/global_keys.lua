...
return gears.table.join(
  ...
  -- Tag browsing
  awful.key({ modkey }, '#113', awful.tag.viewprev, {
    description = 'View previous tag',
    group = 'Tag',
  }),
  awful.key({ modkey }, '#114', awful.tag.viewnext, {
    description = 'View next tag',
    group = 'Tag',
  }),
  awful.key({ modkey }, '#66', awful.tag.history.restore, {
  ...
  awful.key({ modkey }, '#38', function()
    user_vars.hide_bar = not user_vars.hide_bar
  end, {
  ...
  awful.key({ modkey, 'Shift' }, '#46', function()
    ...
    description = 'Increase client width',
    group = 'Layout',
  }),
  awful.key({ modkey, 'Shift' }, '#43', function()
    awful.tag.incmwfact(-0.05)
  end, {
    description = 'Decrease client width',
    group = 'Layout',
  }),
  ...
  --   { descripton = "Client switcher (alt+tab)", group = "Launcher" }),
  ...
  -- awful.key({ "Control" }, "#46", function()
  awful.key({ modkey }, '#48', function()
    awful.util.spawn('/bin/bash -c $HOME/.config/rofi/utility/util.sh')
  end, {
    description = 'Utility',
    group = 'Applications',
  }),
  awful.key({ modkey }, '#26', function()
    ...
    descripton = 'Open file manager',
    group = 'System',
  }),
  awful.key({ modkey, 'Shift' }, '#26', function()
    awesome.emit_signal('module::powermenu:show')
  end, {
    descripton = 'Session options',
    group = 'System',
  }),
  ...
  awful.key({}, 'XF86MonBrightnessUp', function(c)
    ...
    awful.spawn.easy_async_with_shell('pkexec /usr/bin/xfpm-power-backlight-helper --get-brightness', function(stdout)
      awful.spawn.easy_async_with_shell(
        'pkexec /usr/bin/xfpm-power-backlight-helper --set-brightness ' .. tostring(tonumber(stdout) + BACKLIGHT_SEPS),
        function(stdou2) end
      )
    ...
    description = 'Raise backlight brightness',
    group = 'System',
  }),
  awful.key({}, 'XF86MonBrightnessDown', function(c)
    ...
    description = 'Lower backlight brightness',
    group = 'System',
  }),
  ...
  awful.key({ modkey }, '#22', function()
    awful.spawn.easy_async_with_shell([[xprop | grep WM_CLASS | awk '{gsub(/"/, "", $4); print $4}']], function(stdout)
      if stdout then
